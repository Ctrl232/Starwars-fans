...
export function CharacterGrid({ characters, onSelect, onPrefetch, skeletons = 0 }: CharacterGridProps) {
  return (
    <Grid container spacing={2} component="ul" sx={{ listStyle: 'none', p: 0, m: 0 }}>
      {characters.map((character) => (
        <Grid key={character.id} size={GRID_SIZE} component="li">
          <CharacterCard character={character} onSelect={onSelect} onPrefetch={onPrefetch} />
        </Grid>
      ))}
